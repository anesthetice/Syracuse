...
fn main() -> Result<()> {
    ...
    if !_conf_dir.exists() {
        std::fs::create_dir_all(_conf_dir).wrap_err_with(|| format!("Failed to create the config directory at '{}'", _conf_dir.display()))?;
    }
    let _data_dir = dirs.data_dir();
    if !_data_dir.exists() {
        std::fs::create_dir_all(_data_dir).context(format!("Failed to create the data directory at '{}'", _data_dir.display()))?;
    }

    config::CONFIG
        .set(config::Config::load(&dirs.config_dir().join("syracuse.conf")))
